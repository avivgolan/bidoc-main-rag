...
**עדכון — בדיקת עומק (2026-07-03, אחרי הצהריים): המשימה נדחית עד העשרת הגרף בצד ה-ingestion.**
בדיקה ישירה של תוכן הגרף (`listProjectGraph`, 1000 שורות) העלתה:
- סוגי צמתים: alert 675, hashtag 127, event 107, document 12 — **אין צומתי ישות אמיתית** (קבלן/אדם/ארגון/מיקום).
- כל 981 קשתות ה-`mentions` מצביעות על צומתי hashtag בלבד; אפס mentions לישות אמיתית.
- אין אף קשת רשומה↔רשומה; יש רק 18 קשתות `from_document`.
- מבנה הצמתים כן מוכן למיפוי (יש `source_table` + `source_id` על צמתים מגובי-רשומות).

משמעות: קיבוץ דרך הגרף היום שקול לקיבוץ לפי האשטגים — סיגנל שהצנרת כבר משתמשת בו ישירות. מימוש עכשיו יוסיף מורכבות בלי מידע חדש ויסכן over-merging דרך האשטגים גנריים ("עדכון", "פרויקט").
**תנאי מקדים חדש למשימה:** להעשיר את בניית הגרף (צד n8n/ingestion שממלא את `graph_nodes`/`graph_edges`) בחילוץ ישויות אמיתיות — קבלנים, אנשים, ספקים, מיקומים — ורק אז לממש את המיפוי (דרך co-mention של ישות, עם הגנת hub: ישות שמחוברת ליותר מ-6 רשומות אינה סיגנל איחוד).
