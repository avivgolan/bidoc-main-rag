...
### Task A2 — חילוץ event_date מטקסט (lite-LLM, מאחורי דגל)

מיילים/מסמכים/whatsapp שאין להם `event_date` דטרמיניסטי: batch lite-LLM שמחלץ תאריך אירוע מהטקסט עם ציטוט grounding (מאומת בקוד מול הטקסט, תקדים `validateExtractedEntities`), כותב רק כשיש ודאות. דגל `config.indexing.llmEventDates === true` (כבוי). **תלוי ב-A1.** סיכון: בינוני (המצאת תאריכים — ה-grounding חובה).

### Task A3 — אינדוקס מסמכים ללא summary מוכן

יצירת summary/hashtags ב-lite-LLM לשורות מקור שאין להן (כיום n8n עושה זאת). נדרש רק לקראת כיבוי ה-workflow ב-n8n. **תלוי ב-A1 + החלטת כיבוי.**
...
1. **A1** — סוכן האינדוקס + backfill תאריכים (dry-run → אימות → apply → אימות חי).
2. **M1** — תיקון השחתת ההגדרות (קטן, עצמאי).
3. **B1** — כלי שאילתה פנימיים, החל מ-`safety_report`.
4. **A2/A3** — העשרות LLM לאינדוקס, לקראת כיבוי workflow האינדוקס ב-n8n.
5. **Phase C** — רק אחרי החלטת משתמש ותשתית scheduling.

## שאלות פתוחות למשתמש

- `submittals`: אין טבלה ב-Kapaim — למחוק את הכלי או שיש מקור אחר?
- כיבוי workflow האינדוקס ב-n8n: אחרי כמה זמן הרצה מקבילה של A1?
- `toolsRuntime.enabled=false` ו-`alertAgentEnabled=false` בהגדרות החיות — האם הכיבוי של ה-Alert Agent מכוון, או תוצר לוואי של ההשחתה שכדאי להחזיר אחרי M1?
