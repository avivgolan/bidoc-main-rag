...
- Known pre-existing UI bug (NOT from this work): `init()` at app.js top calls `wireWorkflow` before `let _workflowWired` initializes (TDZ ReferenceError caught by safeInitStep); the workflow tab init step fails on every load. Also, the first click on a history item can be swallowed by a concurrent history re-render replacing the buttons — a second click works.
