...
- The current Content `data_index` schema uses `primary_date` for timeline dates and `index_text`/`summary`/`title` for content text, plus `source_url`, `source_table`, and `source_id` for provenance.
...
- 2026-06-03 -- Mapped timeline/RAG previews to the new `data_index` schema fields (`primary_date`, `index_text`, `summary`, `title`, `source_url`).
